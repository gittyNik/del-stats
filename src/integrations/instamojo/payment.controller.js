...
const getPaymentDetails = ({ payment_id }) =>
    ...
    .catch(err => throw new Error(`Unable to fetch payment details for payment id: ${payment_id}`));


const getPaymentDetailsByRequestId = ({ payment_request_id }) =>
	request
...
const checkPaymentStatus = ({ payment_id, payment_request_id }) => {
	if (payment_id) {
		...
		.then(data => data.payment_request.payments[0] ? (data.payment_request.payments[0].status === "Credit" ? "Payment Successful": "Payment Failed"): "No Payment Made")
		.catch(err => throw new Error(`Unable to fetch payment details for payment id: ${payment_id}`));
	}
}
