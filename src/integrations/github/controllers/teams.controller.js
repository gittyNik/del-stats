...
const teamNameFormat = (
  cohort_name, program_id, location, start_date,
) => `${cohort_name}_${toSentenceCase(program_id)}_${location}_${new Date(
  start_date,
).getFullYear()}`;

export const removeMemberFromTeam = (name,
  githubUsername) => octokit.teams.removeMembershipInOrg({
  org,
  team_slug: name,
