...
export const getRecentCommitByUser = (username, repository_name) => octokit.repos
  .listCommits({
    owner: org,
...
export const getRecentCommitInRepository = repository_name => octokit.repos
  .listCommits({
    owner: org,
    repo: repository_name,
  })
...
export const getAllCommits = async repository_name => getNumberOfPages(
  'allCommits', repository_name,
).then(
  async ({ pages }) => {
    let commits = [];
...
export const getAllAuthoredCommits = async (repository_name, author) => getNumberOfPages(
  'authoredCommits', { repository_name, author },
).then(
  async ({ pages }) => {
    let commits = [];
    for (let i = 1; i <= pages; i++) {
