...
export const getSpotterTeam = student => new Promise(async (resolve, reject) => {
  ...
  const cohort = await Cohort.findById(student.currentCohort).exec();
  const spotterTeam = cohort.spotters.find(team => team.students.some(s => student._id.equals(s)));
  if (spotterTeam == null) {
    return resolve(null);
  }
