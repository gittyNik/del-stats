...
module.exports = {
  up: (queryInterface, Sequelize) => queryInterface.sequelize.transaction(async (t) => {
    ...
    const addCohortMilestone = queryInterface.bulkInsert(
      'cohort_milestones', _.times(100, COHORT_MILESTONE),
      { transaction: t },
      {},
    );

    return Promise.all([addProgram, addMilestones, addCohort, addUser, addCohortMilestone])
      .then(() => console.log('Seeded Cohort Milestones'))
  ...
  down: async queryInterface => queryInterface.sequelize.transaction(t => Promise.all([
    ...
    .then(() => console.log('milestones, cohorts, programs, users and cohort Milestones reverted'))
    .catch(err => console.error(err))),
};
